...
//create the spiral svg
  ...
  .on("mouseover", function(d) { moveLovebirds(d[0], d[2], d[3]) })
  .append("svg:title")
...
//Radial label
  var segmentLabels = ["April","March","February","January","December", "November", "October", "September", "August", "July", "June", "May"];
  ...
  labels.selectAll("text")
      ...
      .attr("startOffset", function(d, i) {return i * 100 / 12 + "%";})
...
function moveLovebirds(day, cityManolo, cityDiana){
  ...
  for(i=0;i<=citiesData.length;i++){
    ...
    if(citiesData[i].city == cityManolo){     
      movePerson("manolo", citiesData[i].lon, citiesData[i].lat, -5, citiesData[i].city);
    }//end if
  }//end for
